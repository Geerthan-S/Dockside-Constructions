...
export default async function SeoPage() {
  ...
  return (
    <div className="grid gap-8 xl:grid-cols-[0.9fr_1.1fr]">
      <div>
        <h1 className="text-3xl font-semibold">SEO Metadata Editor</h1>
        <p className="mt-2 text-muted-foreground">Maintain route-level titles, descriptions and social images.</p>
        <div className="mt-8 grid gap-3">
          {entries.map((entry) => (
            <div key={entry.id} className="rounded-md border border-white/10 bg-card/45 p-4">
      ...
      <form action={saveSeo} className="grid gap-4 rounded-lg border border-white/10 bg-card/45 p-6">
